...
def get_loader(args, config, low1, high1, low2, high2):
    if args.trainset == 'DIV2K':
        train_dataset = HR_image(config, config.train_data_dir)
        test_dataset = Datasets(config.test_data_dir)
        batch_size_test = 1
    ...
    # 为两个任务分别生成 SNR 值
    train_snr_task1 = generate_snr_values(num_train_batches, low1, high1, seed=42)
    train_snr_task2 = generate_snr_values(num_train_batches, low2, high2, seed=43)
    test_snr_task1 = generate_snr_values(num_test_batches, low1, high1, seed=44)
    test_snr_task2 = generate_snr_values(num_test_batches, low2, high2, seed=45)

    # 合并两个任务的 SNR
    train_snr_values = list(zip(train_snr_task1, train_snr_task2))
    test_snr_values = list(zip(test_snr_task1, test_snr_task2))
