...
class WITT(nn.Module):
    def __init__(self, args, config):
        ...
        self.H = self.W = 0
        self.multiple_snr_low = args.multiple_snr_low.split(",")
        self.multiple_snr_high = args.multiple_snr_high.split(",")
        for i in range(len(self.multiple_snr_low)):
            self.multiple_snr_low[i] = int(self.multiple_snr_low[i])
        for i in range(len(self.multiple_snr_high)):
            self.multiple_snr_high[i] = int(self.multiple_snr_high[i])
    ...
    def forward(self, input_image, task_name,given_SNR = None):
        ...
        if H != self.H or W != self.W:
            ...
            self.W = W
        if task_name=='low':
            multiple_snr = self.multiple_snr_low
        else:
            multiple_snr = self.multiple_snr_high
        if given_SNR is None:
            SNR = choice(multiple_snr)
            chan_param = SNR
        else:
            chan_param = given_SNR
        chan_param = torch.tensor(chan_param).expand(B).unsqueeze(1).cuda()
