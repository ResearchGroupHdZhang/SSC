...
class Channel(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        # if config.logging:
        #     config.logging.info('【Channel】: Built {} channel, SNR {} dB.'.format(
    ...
    def gaussian_noise_layer(self, input_layer, std, name=None):
        ...
        mean = torch.zeros(input_shape, device=device)
        std_expanded = std.view(-1,1).expand_as(input_layer.view(std.shape[0],-1))
        std_expanded = std_expanded.reshape(-1).to(device)
        noise_real = torch.normal(mean=mean, std=std_expanded)
        # noise_real = torch.normal(mean=0.0, std=std)
        # noise_imag = torch.normal(mean=0.0, std=std, size=np.shape(input_layer), device=device)
        noise_imag = torch.normal(mean=mean, std=std_expanded)
